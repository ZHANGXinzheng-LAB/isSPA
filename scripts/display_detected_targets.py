...
def main():
    ...
    with open(star_file, 'r') as f:
        lines = f.readlines()
    data_lines = lines[star_format[-2]:]
    # 区分RELION-3.0/3.1
    if len(star_format) == 5:
        meta_lines2 = lines[star_format[-2]-star_format[1]:star_format[-2]]
    else:
        meta_lines2 = lines[star_format[-2]-star_format[0]:star_format[-2]]

    # 读取坐标编号
    for i in meta_lines2:
        if i.split('#')[0] == '_rlnCoordinateX ':
            x_n = int(i.split('#')[1])
        if i.split('#')[0] == '_rlnCoordinateY ':
            y_n = int(i.split('#')[1])
    ...
    for i in data_lines:
        if i.strip():
            ...
            x = new_line.split()[x_n]
            y = new_line.split()[y_n]
            if name != name_record:
