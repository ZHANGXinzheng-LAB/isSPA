...
def main():
    ...
    output = instar.split('.star')[0] + output
    ...
    meta_label_list2.extend(['_rlnOpticsGroup', '_rlnOriginXAngst', '_rlnOriginYAngst'])
    ...
    for i in range(n_mlines+1, len(instar_line)):
        if(instar_line[i].split()):
            ...
            for j in meta_label_num_list2:
                ...
                tmp += " "
            #subset = random.randint(1,2)
            tmp += f" 1 0 0 \n"
            data += tmp
    with open(output, 'a+') as o1:
        o1.write(data)
    print(f"All particles data are stored in {output}\nYou can look at the results in the display GUI")
    

def parse_command_line():
    #usage="%prog <input relion 3.1 star> <output relion 3.0 star>"
    parser = argparse.ArgumentParser(description='isSPA_1.1.3 by Zhang Lab at Institute of Biophysics, Chinese Academy of Sciences\nConvert RELION-3.0 star file to RELION-3.1 star file')
